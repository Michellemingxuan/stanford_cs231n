...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        print(self.state_dict()['pe'].size())
        print(x.size())
        output = self.dropout(x + self.state_dict()['pe'][:, :S, :])
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        scores = torch.matmul(query.permute(0, 2, 1, 3), key.permute(
            0, 2, 3, 1)) / math.sqrt(D/self.h)
        ...
        output = torch.matmul(attns, value.permute(
            0, 2, 1, 3)).permute((0, 2, 1, 3)).contiguous().view(N, S, D)
        # (N, H, S,T) * (N, H, T, E/H) -> (N, H, S, E/H) -> (N, S, E)
