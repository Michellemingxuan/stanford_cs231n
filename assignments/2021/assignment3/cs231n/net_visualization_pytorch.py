...
def compute_saliency_maps(X, y, model):
    ...
    output = class_scores.gather(1, y.view(-1, 1)).squeeze().sum()
    output.backward()
    with torch.no_grad():
...
def make_fooling_image(X, target_y, model):
    ...
    model.eval()
    for i in range(100):
        class_scores = model(X_fooling)
        predict_y = class_scores.data.max(1)[1][0].item()
        if predict_y == target_y:
            break
        else:
            target_class_score = class_scores[:, target_y]
            target_class_score.backward()
            with torch.no_grad():
                X_fooling += learning_rate * X_fooling.grad / \
                    torch.norm(X_fooling.grad)
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    model.eval()
    # img.requires_grad_()
    target_score = model(img)[:, target_y] - l2_reg * torch.norm(img).square()
