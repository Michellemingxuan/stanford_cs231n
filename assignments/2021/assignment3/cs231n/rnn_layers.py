...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = (np.exp(affine) - np.exp(-affine)) / \
        (np.exp(affine) + np.exp(-affine))
    cache = {"x": x, "prev_h": prev_h, "Wx": Wx, "Wh": Wh, "affine": affine}
    pass
...
def rnn_step_backward(dnext_h, cache):
    ...
    daffine = dnext_h * 4 / (np.exp(affine) + np.exp(-affine)) ** 2
    db = np.sum(daffine, axis=0)
    dx = daffine.dot(cache["Wx"].T)
    dprev_h = daffine.dot(cache["Wh"].T)
    dWx = cache["x"].T.dot(daffine)
    dWh = cache["prev_h"].T.dot(daffine)
    pass
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        next_h, cache[i] = rnn_step_forward(
            x[:, i, :], prev_h, Wx, Wh, b)
        h[:, i, :] = next_h
        prev_h = next_h
    pass
...
def rnn_backward(dh, cache):
    ...
    D = cache[0]["x"].shape[1]
    dx = np.zeros((N, T, D))
    dx[:, T-1, :], dprev_h, dWx, dWh, db = rnn_step_backward(
        dh[:, T-1, :], cache[T-1])

    # print(dx.shape)

    for i in range(T-2, -1, -1):
        dx[:, i, :], dprev_h, dWx_temp, dWh_temp, db_temp = rnn_step_backward(
            dh[:, i, :] + dprev_h, cache[i])
        dWx += dWx_temp
        dWh += dWh_temp
        db += db_temp
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = {"Wx": Wx, "Wh": Wh, "x": x, "prev_h": prev_h, "a_i": a_i, "a_f": a_f, "a_o": a_o,
             "a_g": a_g, "i": i, "f": f, "o": o, "g": g, "next_c": next_c, "prev_c": prev_c}
    pass
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dc = cache["o"] * dnext_h * (1 - np.tanh(cache["next_c"]) ** 2) + dnext_c
    dprev_c = cache["f"] * dc
    df = cache["prev_c"] * dc
    di = cache["g"] * dc
    dg = cache["i"] * dc
    da_i = cache["i"] * (1 - cache["i"]) * di
    da_f = cache["f"] * (1 - cache["f"]) * df
    da_o = cache["o"] * (1 - cache["o"]) * do
    da_g = (1 - cache["g"] ** 2) * dg
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        next_h, next_c, cache[i] = lstm_step_forward(
...
def lstm_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, T, H = dh.shape
    D = cache[0]["Wx"].shape[0]
    dx = np.zeros((N, T, D))
    dnext_c = np.zeros((N, H))
    dx[:, T-1, :], dprev_h, dprev_c, dWx, dWh, db = lstm_step_backward(
        dh[:, T-1, :], dnext_c, cache[T-1])

    for i in range(T-2, -1, -1):
        # print(i)
        dnext_c = dprev_c
        dx[:, i, :], dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(
            dh[:, i, :] + dprev_h, dnext_c, cache[i])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
    dh0 = dprev_h
