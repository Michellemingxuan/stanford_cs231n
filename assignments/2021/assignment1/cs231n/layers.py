...
def svm_loss(x, y):
    ...
    margin = x - np.sum(x * mask, axis=1).reshape((num_train, 1)) + 1
    loss = np.sum(margin * (margin > 0) * ~mask) / num_train

    dx = (margin > 0) * ~mask - ((num_class - 1) -
                                 np.sum(margin < 0, axis=1).reshape((num_train, 1))) * mask
    dx = dx / num_train
    pass
